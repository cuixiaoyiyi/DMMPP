...
public class DMMMain {

	public static void main(String[] args) {
		DMMSootConfig.ANDROID_PLATFORM_PATH = args[0];
		DMMSootConfig.ApkPath = args[1];
		DMMSootConfig.output = args[2];
...
//		DDMSootConfig.output = ".\\output";
		DMMSootConfig.sootInitialization();
...
//			}	
		...
		String apkName = DMMSootConfig.ApkPath.substring(DMMSootConfig.ApkPath.lastIndexOf(File.separator ) + 1);
		File file = new File(DMMSootConfig.output + File.separator + apkName);
		if(file.exists()) {
			file.delete();
		}
		...
		if (Options.v().output_format() == Options.output_format_dex) {
			file = new File(DMMSootConfig.output + File.separator + apkName);
			if(file.exists()) {
				file.renameTo(new File(DMMSootConfig.output + File.separator + "DMMPP_"+apkName));
			}
		}
